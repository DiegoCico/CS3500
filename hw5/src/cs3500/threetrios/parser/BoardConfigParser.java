...
/**
 * fetching the info from the config.
 */
public class BoardConfigParser {
   ...
   */
  public static GameModel parseBoardConfig() throws FileNotFoundException {
    File file = new File("board.config");
    ...
    while (scanner.hasNextLine()) {
      ...
      cardCellCount = getCurrentRow(parsingGridLayout, currentRow, rows, line, grid, cardCellCount);
    ...
    if (cards.size() < cardCellCount + 1) {
      throw new IllegalStateException("Insufficient number of cards. Required: "
              + (cardCellCount + 1));
    }
