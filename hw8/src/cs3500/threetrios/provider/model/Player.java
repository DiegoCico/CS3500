...
public enum Player {
   ...
   */
  public static Map<Player, List<CardAdapter>> splitCardsBetweenPlayers(List<CardAdapter> cards) {
    if (cards == null) {
      throw new IllegalArgumentException("cards cannot be null");
    }

    for (CardAdapter card : cards) {
      if (card == null) {
        throw new IllegalArgumentException("all cards within cards cannot be null");
      }
    }

    Map<Player, List<CardAdapter>> result = new HashMap<>();
    List<CardAdapter> bCards = new ArrayList<>(cards.subList(0, cards.size() / 2));
    for (CardAdapter card : bCards) {
      card.switchPlayer(Player.B);
    }

    List<CardAdapter> aCards = new ArrayList<>(cards.subList(cards.size() / 2, cards.size()));
    for (CardAdapter card: aCards) {
